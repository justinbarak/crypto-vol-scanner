...
from multi_nomics_bt_strategy_focused import VolumeStrategy
    ...
    return_df = df.astype(
        {
            ...
            "buy_avg_volume_before_trigger": "float32",
        },
        copy=True,
    )
...
def main() -> None:
    ...
    try:
        ...
        for currency in progress_bar(
            ...
            for VolAvgParam in [20]:
                for TriggerVolParam in [6]:
                    for transparent in [True]:
                        task_queue.put(
                            (
                                {
                                    "currency": currency,
